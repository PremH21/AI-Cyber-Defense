...
def load_split(name):
    df = pd.read_parquet(os.path.join(DATA_DIR, f"cicids_multiclass_{name}.parquet"))
...
def main():
    ...
    joblib.dump(rf, os.path.join(MODEL_DIR, "rf_multiclass_cicids.joblib"))
    ...
    joblib.dump(xgb_model, os.path.join(MODEL_DIR, "xgb_multiclass_cicids.joblib"))
